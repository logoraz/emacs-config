;;; gx-vibe.el --- Vibe Coding with LLMA -*- lexical-binding: t -*-
...
(use-package gptel
  ...
  :bind (("C-c g" . gptel-send)
         ("C-c G" . gptel-menu)
         ("C-c m" . gx/gptel-select-model)
         ("C-c r" . gx/gptel-send-region-or-buffer))
  :hook ((gptel-mode . visual-line-mode))
  :config
  (setq gptel-model 'claude-sonnet-4-20250514
        gptel-backend (gptel-make-gh-copilot "Copilot"))

  (defun gx/gptel-send-region-or-buffer ()
  ...
  (defun gx/fill-gptel-buffer (&rest _)
    "Force-fill the current `gptel` buffer after a response.

Use with :hook ((gptel-post-response . gx/fill-gptel-buffer))
"
    (interactive)
